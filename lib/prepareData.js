...
function prepareData(applications, x, y, getKey) {
    ...
    for (var _i = 0, applications_1 = applications; _i < applications_1.length; _i++) {
        ...
        var _loop_1 = function (use) {
            ...
            if (yIndex !== -1 && xIndex !== -1 && !result[yIndex][xIndex].some(function (da) { return da.key === key; })) {
                result[yIndex][xIndex].push({ key: key, detail: app.detail, dimensions: use.dimensions, commissioned: use.commissioned, decommissioned: use.decommissioned, status: use.status });
            }
        };
