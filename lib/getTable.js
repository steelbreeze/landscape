 ...
 */
function getTable(applications, x, y, splitOnY) {
    ...
    var topRow = __spreadArrays([cell("", "xyAxis")], x.values.map(function (xValue) { return cell(xValue, "xAxis"); }));
    if (splitOnY) {
        ...
        applications.forEach(function (row, rowIndex) {
            var _loop_1 = function (rowSplitIndex) {
                ...
                result.push(__spreadArrays([cell(y.values[rowIndex], "yAxis")], row.map(function (apps, columnIndex) {
                    var app = apps[Math.floor(rowSplitIndex * appCounts[rowIndex][columnIndex] / rowSplits_1[rowIndex])];
                    return app ? cell(app.key.major, app.key.minor, rowSplits_1[rowIndex]) : cell("", "", rowSplits_1[rowIndex]);
                })));
            };
        ...
        topRow.forEach(function (app, index) {
            for (var i = 0; i < colSplits_1[index]; i++) {
                rr_1.push(cell(app.text, app.style, 1, colSplits_1[index]));
            }
        });
        result.push(rr_1);
        applications.forEach(function (row, rowIndex) {
            var rr = [cell(y.values[rowIndex], "yAxis")];
            row.forEach(function (apps, colIndex) {
                for (var i = 0; i < colSplits_1[colIndex + 1]; i++) {
                    ...
                    rr.push(app_1 ? cell(app_1.key.major, app_1.key.minor, 1, colSplits_1[colIndex + 1]) : cell("", "", 1, colSplits_1[colIndex + 1]));
                }
            });
            result.push(rr);
        });
 ...
 * @hidden
 */
function cell(text, style, rowSplit, colSplit) {
    if (rowSplit === void 0) { rowSplit = 1; }
    if (colSplit === void 0) { colSplit = 1; }
    return { text: text, style: style, cols: 1, rows: 1, height: 1 / rowSplit, width: 1 / colSplit };
}
/**
