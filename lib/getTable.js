...
function getTable(applications, x, y) {
    ...
    var flattened = applications.reduce(function (result, app) { return __spreadArrays(result, app.usage.map(function (use) { return { detail: app.detail, xValue: use[x.name], yValue: use[y.name], status: use.status }; })); }, new Array());
