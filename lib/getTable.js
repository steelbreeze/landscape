 ...
 */
function getTable(applications, x, y, splitOnY, key) {
    ...
    var topRow = __spreadArrays([cell(detail(key, ""), "xyAxis")], x.values.map(function (xValue) { return cell(detail(key, xValue), "xAxis"); }));
    if (splitOnY) {
        ...
        applications.forEach(function (row, rowIndex) {
            var _loop_1 = function (rowSplitIndex) {
                ...
                result.push(__spreadArrays([cell(detail(key, y.values[rowIndex]), "yAxis")], row.map(function (apps, columnIndex) {
        ...
        result.push(rr_1);
        applications.forEach(function (row, rowIndex) {
            var rr = [cell(detail(key, y.values[rowIndex]), "yAxis")];
