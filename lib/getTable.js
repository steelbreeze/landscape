...
function getTable(flattened, x, y) {
    ...
    var _loop_1 = function (yValue) {
        ...
        var cellCounts = row.map(function (cell) { return cell.length || 1; });
        var split = cellCounts.reduce(leastCommonMultiple, 1);
        var _loop_2 = function (y_1) {
            result.push(__spreadArrays([Cell({ id: "", name: yValue }, "yAxis")], row.map(function (apps, x) {
                var app = apps[Math.floor(y_1 / split * cellCounts[x])];
