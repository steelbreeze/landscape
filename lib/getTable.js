...
function getTable(applications, axes) {
    ...
    var interim = __spreadArrays([xAxis], axes.yValues.map(function (y) { return __spreadArrays([[new Cell_1.Cell(Detail_1.noDetail(y), "yAxis")]], axes.xValues.map(function (x) { return flattened.filter(function (a) { return a.x === x && a.y === y; }).map(function (a) { return new Cell_1.Cell(a.detail, a.status); }); })); }));
