...
function getTable(applications, x, y) {
    ...
    var flattened = selectMany_1.selectMany(applications, function (app) { return app.usage; }, function (use, app) { return { detail: app.detail, xValue: use.dimensions[x.name], yValue: use.dimensions[y.name], status: use.status }; });
    var xAxis = __spreadArrays([[new Cell_1.Cell(new Detail_1.Detail(), "xAxis")]], x.values.map(function (xValue) { return [new Cell_1.Cell(new Detail_1.Detail("", xValue), "xAxis")]; }));
    var interim = __spreadArrays([xAxis], y.values.map(function (yValue) { return __spreadArrays([[new Cell_1.Cell(new Detail_1.Detail("", yValue), "yAxis")]], x.values.map(function (xValue) { return flattened.filter(function (app) { return app.xValue === xValue && app.yValue === yValue; }).map(function (app) { return new Cell_1.Cell(app.detail, app.status); }); })); }));
    var _loop_1 = function (iY) {
        ...
        for (var iX = interim[iY].length; iX--;) {
            if (interim[iY][iX].length === 0) {
                interim[iY][iX].push(new Cell_1.Cell(new Detail_1.Detail(), "empty"));
            }
        }
