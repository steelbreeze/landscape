...
function getTable(flattened, x, y) {
    // create a 3D jagged array of y-axis, x-axis and appliactions featuring in the x/y cell
    var tab = y.values.map(function (yValue) { return x.values.map(function (xValue) { return flattened.filter(function (app) { return app.yValue === yValue; }).filter(function (app) { return app.xValue === xValue; }); }); });
