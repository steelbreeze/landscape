...
function getTable(applications, axes) {
    ...
    var flattened = applications.reduce(function (result, app) { return __spreadArrays(result, app.usage.map(function (use) { return { detail: app.detail, xValue: use[axes.xDimension], yValue: use[axes.yDimension], status: use.status }; })); }, new Array());
    ...
    var xAxis = __spreadArrays([[new Cell_1.Cell(Detail_1.noDetail(), "xAxis")]], axes.xValues.map(function (x) { return [new Cell_1.Cell(Detail_1.noDetail(x), "xAxis")]; }));
    var interim = __spreadArrays([xAxis], axes.yValues.map(function (y) { return __spreadArrays([[new Cell_1.Cell(Detail_1.noDetail(y), "yAxis")]], axes.xValues.map(function (x) { return flattened.filter(function (a) { return a.xValue === x && a.yValue === y; }).map(function (a) { return new Cell_1.Cell(a.detail, a.status); }); })); }));
