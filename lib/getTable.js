 ...
 */
function getTable(applications, axes, rows) {
    if (rows === void 0) { rows = true; }
    var result = [];
    ...
    var topRow = __spreadArrays([cell({ id: "", name: "" }, "yAxis")], axes.x.values.map(function (xValue) { return cell({ id: "", name: xValue }, "xAxis"); }));
    if (rows) {
        ...
        applications.forEach(function (row, rowIndex) {
            var _loop_1 = function (rowSplitIndex) {
                ...
                result.push(__spreadArrays([cell({ id: "", name: axes.y.values[rowIndex] }, "yAxis")], row.map(function (apps, columnIndex) {
                    ...
                    return app ? cell(app.detail, app.status, 1, rowSplits_1[rowIndex]) : cell({ id: "", name: "" }, "empty", 1, rowSplits_1[rowIndex]);
                })));
            };
        ...
        result.push(rr_1);
        applications.forEach(function (row, rowIndex) {
            var rr = [cell({ id: "", name: axes.y.values[rowIndex] }, "yAxis")];
            row.forEach(function (apps, colIndex) {
                for (var i = 0; i < colSplits_1[colIndex + 1]; i++) {
                    ...
                    rr.push(app_1 ? cell(app_1.detail, app_1.status, 1, colSplits_1[colIndex + 1]) : cell({ id: "", name: "" }, "empty", 1, colSplits_1[colIndex + 1]));
                }
            });
            result.push(rr);
        });
