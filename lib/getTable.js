...
function getTable(applications, x, y) {
    ...
    applications.forEach(function (row, rowIndex) {
        ...
        var appsPerCell = row.map(function (apps) { return apps.length; });
        ...
        var _loop_1 = function (rowSplitIndex) {
            ...
            result.push(__spreadArrays([cell(heading(y.values[rowIndex]), "yAxis")], row.map(function (apps, columnIndex) {
        ...
        for (var rowSplitIndex = 0; rowSplitIndex < rowSplit; rowSplitIndex++) {
