 ...
 */
function getTable(applications, axes, splitOnY) {
    ...
    var topRow = __spreadArrays([cell({ id: "", name: "" }, "xyAxis")], axes.x.values.map(function (xValue) { return cell({ id: "", name: xValue }, "xAxis"); }));
    if (splitOnY) {
        ...
        applications.forEach(function (row, rowIndex) {
            var _loop_1 = function (rowSplitIndex) {
                ...
                result.push(__spreadArrays([cell({ id: "", name: axes.y.values[rowIndex] }, "yAxis")], row.map(function (apps, columnIndex) {
                    ...
                    return app ? cell(app.detail, app.status, rowSplits_1[rowIndex], 1) : cell({ id: "", name: "" }, "empty", rowSplits_1[rowIndex], 1);
                })));
            };
