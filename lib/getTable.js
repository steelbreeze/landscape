...
function getTable(flattened, x, y) {
    ...
    var xAxis = __spreadArrays([[new Cell_1.Cell({ id: "", name: "" }, "xAxis")]], x.values.map(function (xValue) { return [new Cell_1.Cell({ id: "", name: xValue }, "xAxis")]; }));
    var interim = __spreadArrays([xAxis], y.values.map(function (yValue) { return __spreadArrays([[new Cell_1.Cell({ id: "", name: yValue }, "yAxis")]], x.values.map(function (xValue) { return flattened.filter(function (app) { return app.xValue === xValue && app.yValue === yValue; }).map(function (app) { return new Cell_1.Cell(app.detail, app.status); }); })); }));
    var _loop_1 = function (iY) {
        ...
        for (var iX = interim[iY].length; iX--;) {
            if (interim[iY][iX].length === 0) {
                interim[iY][iX].push(new Cell_1.Cell({ id: "", name: "" }, "empty"));
            }
        }
