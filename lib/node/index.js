...
function table(cube, axes, getKey, onX, precise = false) {
    ...
    return split(cube.map(row => row.map(table => table.length ? cells(table, getKey) : [cell(makeKey('empty'))])), axes, onX, precise);
}
exports.table = table;
/**
 ...
 * @hidden
 */
const keyEquals = (a, b) => a.key === b.key && a.value === b.value && a.style === b.style;
/**
 * Creates a key within a table.
 * @hidden
 */
const makeKey = (style, value = '') => ({ key: '', value, style });
/**
