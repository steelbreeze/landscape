...
function splitX(cube, yAxis, xAxis, getKey) {
    ...
    const header = generate(xAxis[0].pairs.length, row => [...xyHeaderRow(yAxis), ...xAxis.reduce((res, c, index) => [...res, ...generate(xSplits[index], () => cell({ className: `axis x ${c.pairs[row].key}`, text: c.pairs[row].value }))], [])]);
