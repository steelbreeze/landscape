...
const table = (cube, axes, getElement, onX, method = Math.max) => {
    ...
    const matrix = cube.map(row => row.map(table => table.length ? cells(table, getElement) : [cell('empty')]));
    // calcuate the x splits required (y splits inlined below)
    const xSplits = axes.x.map((_, iX) => onX ? method(...matrix.map(row => row[iX].length)) : 1);
