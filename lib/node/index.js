...
function table(cube, axes, getElement, onX, precise = false) {
    ...
    return expand(cube.map(row => row.map(table => table.length ? cells(table, getElement) : [cell(element('empty'))])), axes, onX, splitter(precise));
}
exports.table = table;
/**
 ...
 * @hidden
 */
function expand(cells, axes, onX, splitter) {
 ...
 * @hidden
 */
const splitter = (precise) => precise ? leastCommonMultiple : Math.max;
/**
