 ...
 * @hidden
 */
const transform = (cube, getElement) => cube.map(slice => slice.map(table => table.length ? cells(table, getElement) : [cell('empty')]));
/**
 * Transform an array of rows into an array of cells.
 * @hidden
 */
const cells = (table, getElement) => table.reduce((result, row, index) => {
