...
function table(cube, xAxis, yAxis, getKey, onX) {
    ...
    return ySplits.reduce((result, ySplit, yIndex) => [...result, ...generate(ySplit, nyi => xSplits.reduce((result, xSplit, xIndex) => [...result, ...generate(xSplit, nxi => {
            ...
            })], yAxis[yIndex].data.map(pair => cell({ className: `axis y ${pair.key}`, text: pair.value }))))], generate(xAxis[0].data.length, row => xSplits.reduce((result, xSplit, xIndex) => [...result, ...generate(xSplit, () => cell({ className: `axis x ${xAxis[xIndex].data[row].key}`, text: xAxis[xIndex].data[row].value }))], yAxis[0].data.map(() => cell({ className: 'axis xy', text: '' })))));
}
exports.table = table;
/**
