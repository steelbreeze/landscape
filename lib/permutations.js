...
function permutations(source) {
    if (source.length === 1) {
        ...
        return selectMany_1.selectMany(source, function (exclude) { return permutations(source.filter(function (element) { return element !== exclude; })); }, function (elements, exclude) { return __spreadArrays([exclude], elements); });
    }
}
