...
function permutations(source) {
    if (source.length === 1) {
        ...
        return selectMany_1.selectMany(source, function (exclude, excludeIndex) { return permutations(source.filter(function (element, elementIndex) { return elementIndex !== excludeIndex; })); }, function (elements, exclude) { return __spreadArrays([exclude], elements); });
    }
}
