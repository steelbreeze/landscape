 ...
 */
function getOptimalAxesBruteForce(applications, axes, axesSelector, xF, yF) {
    ...
    var denormalised = denormalise(applications, axes.x, axes.y);
    var scenarios = [];
    var bestAdjacency = -1;
    // iterate all X and Y using the formulas provided
    for (var _i = 0, _a = yF(axes.y); _i < _a.length; _i++) {
        var yAxisValues = _a[_i];
        for (var _b = 0, _c = xF(axes.x); _b < _c.length; _b++) {
            var xAxisValues = _c[_b];
            var adjacency = countAdjacency(denormalised, xAxisValues, yAxisValues, true, true);
...
exports.getOptimalAxesBruteForce = getOptimalAxesBruteForce;
/**
...
function getOptimalAxes(applications, axes, axesSelector, xF, yF) {
    ...
    for (var _i = 0, _a = (isXLong ? xF : yF)(longAxis); _i < _a.length; _i++) {
        ...
        var adjacency = countAdjacency(denormalised, shortAxis.values, longAxisValues, false, true);
        // just keep the best scenarios
        if (adjacency >= bestAdjacency) {
            // reset the best if needed
            if (adjacency > bestAdjacency) {
    ...
    for (var _b = 0, interimScenarios_1 = interimScenarios; _b < interimScenarios_1.length; _b++) {
        ...
        for (var _c = 0, _d = (isXLong ? yF : xF)(shortAxis); _c < _d.length; _c++) {
            ...
            var adjacency = countAdjacency(denormalised, shortAxisValues, longAxisValues, true, false);
            // just keep the best scenarios
            if (adjacency >= bestAdjacency) {
                // reset the best if needed
                if (adjacency > bestAdjacency) {
