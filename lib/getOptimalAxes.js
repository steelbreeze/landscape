 ...
 */
function getOptimalAxes(applications, x, y, getKey, axesSelector, xF, yF) {
    ...
    var isXLong = x.values.length > y.values.length;
    var shortAxis = isXLong ? y : x;
    var longAxis = isXLong ? x : y;
    ...
    var denormalised = applications.reduce(function (result, app) {
        ...
        var _loop_3 = function (use) {
            ...
            var denormalisedApp = interim.filter(function (a) { return a.key.minor === key.minor; })[0];
            if (!denormalisedApp) {
                denormalisedApp = { detail: app.detail, key: key, usage: [] };
    ...
    for (var _i = 0, _a = (isXLong ? xF : yF)(longAxis); _i < _a.length; _i++) {
        ...
        var _loop_1 = function (app) {
            ...
            var matrix = longAxisValues.map(function (l) { return shortAxis.values.map(function (s) { return app.usage.some(function (use) { return use.l === l && use.s === s; }); }); });
    ...
    for (var _c = 0, interimScenarios_1 = interimScenarios; _c < interimScenarios_1.length; _c++) {
        ...
        var _loop_2 = function (shortAxisValues) {
            ...
            var _loop_4 = function (app) {
                // create 2d boolean matrix where the application exists 
                var matrix = longAxisValues.map(function (l) { return shortAxisValues.map(function (s) { return app.usage.some(function (use) { return use.l === l && use.s === s; }); }); });
