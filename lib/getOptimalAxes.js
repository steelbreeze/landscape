 ...
 */
function getOptimalAxes(applications, x, y, yF, xF) {
    if (yF === void 0) { yF = permutations_1.permutations; }
    if (xF === void 0) { xF = permutations_1.permutations; }
    var result = [];
    var bestAdjacency = -1;
    ...
    var _loop_1 = function (app) {
        var _loop_3 = function (use) {
            var interimApp = interim.filter(function (a) { return a.name === app.detail.name && a.status === use.status; })[0];
            if (!interimApp) {
    ...
    var xPerms = xF(x.values);
    // iterate all X and Y using the formulas provided
    for (var _a = 0, _b = yF(y.values); _a < _b.length; _a++) {
        var yValues = _b[_a];
