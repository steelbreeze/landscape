...
function getGoodAxes(applications, axes, axesSelector, xF, yF) {
    ...
    var xIsLong = axes.x.values.length > axes.y.values.length;
    var shortAxis = xIsLong ? axes.y : axes.x;
    var longAxis = xIsLong ? axes.x : axes.y;
    ...
    for (var _i = 0, _a = (xIsLong ? xF : yF)(longAxis); _i < _a.length; _i++) {
        var yValues = _a[_i];
        var adjacency = countAdjacency(denormalised, shortAxis.values, yValues, false, true);
        // just keep the best scenarios
        if (adjacency >= bestAdjacency) {
            // reset the best if needed
            if (adjacency > bestAdjacency) {
    ...
    for (var _b = 0, lScenarios_1 = lScenarios; _b < lScenarios_1.length; _b++) {
        ...
        for (var _c = 0, _d = (xIsLong ? yF : xF)(shortAxis); _c < _d.length; _c++) {
            var xValues = _d[_c];
