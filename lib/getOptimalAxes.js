...
function getGoodAxes(applications, axes, axesSelector, xF, yF) {
    ...
    for (var _i = 0, _a = (isXLong ? xF : yF)(longAxis); _i < _a.length; _i++) {
        var lValues = _a[_i];
        var adjacency = countAdjacency(denormalised, shortAxis.values, lValues, false, true);
        // just keep the best scenarios
        if (adjacency >= bestAdjacency) {
            // reset the best if needed
            if (adjacency > bestAdjacency) {
    ...
    for (var _b = 0, lScenarios_1 = lScenarios; _b < lScenarios_1.length; _b++) {
        var lValues = lScenarios_1[_b];
        for (var _c = 0, _d = (isXLong ? yF : xF)(shortAxis); _c < _d.length; _c++) {
            var sValues = _d[_c];
            var adjacency = countAdjacency(denormalised, sValues, lValues, true, false);
            // just keep the best scenarios
            if (adjacency >= bestAdjacency) {
                // reset the best if needed
                if (adjacency > bestAdjacency) {
