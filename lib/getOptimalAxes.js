...
function getGoodAxes(applications, axes, axesSelector, xF, yF) {
    ...
    var swapXY = axes.x.values.length > axes.y.values.length;
    var axesToTest = swapXY ? { x: axes.y, y: axes.x } : axes;
    /*
        const lF = swapXY ? xF : yF;
        const sF = swapXY ? xF : yF;
    
        // also swap the x and y axes scenerio generators if necessary
        if(swapXY) {
            const tF = yF;
            yF = xF;
            xF = tF;
    ...
    */
    var denormalised = denormalise(applications, axesToTest);
    // retain only the scenarios with the best adjacency
    var lScenarios = [];
    var scenarios = [];
    var bestAdjacency = -1;
    // iterate the long axis using the provided short axis
    for (var _i = 0, _a = (swapXY ? xF : yF)(axesToTest.y); _i < _a.length; _i++) {
        var yValues = _a[_i];
        var adjacency = countAdjacency(denormalised, axesToTest.x.values, yValues, false, true);
        // just keep the best scenarios
        if (adjacency >= bestAdjacency) {
            // reset the best if needed
            if (adjacency > bestAdjacency) {
    ...
    for (var _b = 0, lScenarios_1 = lScenarios; _b < lScenarios_1.length; _b++) {
        var yValues = lScenarios_1[_b];
        for (var _c = 0, _d = (swapXY ? yF : xF)(axesToTest.x); _c < _d.length; _c++) {
            var xValues = _d[_c];
