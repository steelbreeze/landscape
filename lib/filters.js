...
function filter(applications, useCriteria, appCriteria) {
    if (useCriteria === void 0) { useCriteria = undefined; }
    if (appCriteria === void 0) { appCriteria = undefined; }
    return applications.filter(appCriteria || (function () { return true; })).map(function (app) { return { detail: app.detail, usage: app.usage.filter(useCriteria || (function () { return true; })) }; }).filter(function (app) { return app.usage.length > 0; });
}
