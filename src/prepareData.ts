 ...
 * @returns Returns a 2D array representing the chosen axis; each cell containing an array of the applications used in that context.
 */
export function prepareData3(applications: Array<IApplication & IUsage>, axes: IAxes): Array<Array<Array<IApplication & IUseDetail>>> {
	// create the empty destination table structure
	const result: Array<Array<Array<IApplication & IUseDetail>>> = axes.x.values.map(() => axes.y.values.map(() => []));
